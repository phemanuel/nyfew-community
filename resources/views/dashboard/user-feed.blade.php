            ...
            <div class="flex items-center w-full justify-between border-t p-3">
                ...
                <div class="flex space-x-2">
                    <a href="#" class="bg-red-100 flex font-medium h-9 items-center justify-center px-5 rounded-md text-red-600 text-sm">
                        ...
                        Live </a>
                    <a href="javascript:void(0)" id="share-post-btn" class="bg-blue-600 flex h-9 items-center justify-center rounded-md text-white px-5 font-medium">
                        Share
                    </a>    
                </div>
    ...
    $('.like-btn').on('click', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                // Update like count
                $('.like-btn[data-post-id="' + postId + '"] .like-count').text(response.likeCount);

                // Update avatars and summary
                $('#likes-section-' + postId).html(response.updatedLikesHtml);
            },
            error: function (xhr) {
...
<script>
    $('#share-post-btn').on('click', function(e) {
        ...
        $.ajax({
            ...
            processData: false,
            beforeSend: function() {
                $('#share-post-btn').text('Posting...').prop('disabled', true);
            },
            success: function(response) {
                UIkit.notification({ message: 'Post created successfully!', status: 'success' });
                $('#create-post-modal').removeClass('uk-open'); // hide modal
                $('#create-post-form')[0].reset(); // clear form
                $('#share-post-btn').text('Share').prop('disabled', false);
                // optionally reload posts or append new one
            },
            error: function(xhr) {
